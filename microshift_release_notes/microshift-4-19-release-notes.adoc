...
With this release, configurable transport layer protocols (TLS) on internal control plane components are enabled to help prevent known insecure protocols, ciphers, or algorithms to access the applications you run on {microshift-short}. You can choose to use either the TLS 1.2 or TLS 1.3 security profiles with {microshift-short}. For more information, see
//xref:../microshift_configuring/microshift_auth_security/microshift-tls-config.adoc#microshift-tls-config[Configuring TLS security profiles].
