...
[role="_abstract"]
As a platform engineer or cluster administrator, you must deploy a `Gateway` object in your {ocp} cluster to begin setting up the infrastructure used by application developers. The `Gateway` is the instantiation of your entry point. It tells the controller to provision load balancer with specific ports and security credentials.
