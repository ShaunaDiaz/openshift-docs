...
Update procedures specific to {microshift-short} include planning for edge devices. If you do not re-install devices from scratch, you must use a workaround for UID and GID drift on edge devices during the migration process. One such workaround is to add systemd units that apply the necessary fixes by running before the affected system services.
