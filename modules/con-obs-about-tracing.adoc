...
//is any tracing on by default, or do we have to configure it? if yes, what are the defaults?
...
You can configure both types of tracing in one procedure by configuring the `kuadrant` custom resource (CR). This configuration sends traces to the same collector, providing a complete view of your {prodname} system from policy reconciliation to request processing.
