...
* Create it manually.
* Use a non-delegating DNS policy at a gateway with routes attached. The `kuadrant-operator` creates `DNSRecord` CRs with the secret.
//is the "kuadrant operator" the same thing as connectivity link?
* Use a delegating DNS policy at a gateway. When delegating, the Operator creates a `DNSRecord` CR without a secret reference, but with the default secret of `coredns` type. The authoritative `DNSRecord` applies a default provider secret.
