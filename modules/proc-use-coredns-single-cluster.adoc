...
[source,bash]
----
$ kubectl --context $CTX_PRIMARY -n kuadrant-coredns patch deployment kuadrant-coredns --patch '{"spec":{"template":{"spec":{"volumes":[{"name":"config-volume","configMap":{"name":"coredns-kuadrant-config","items":[{"key":"Corefile","path":"Corefile"}]}}]}}}}'
----
+
...
[source,bash]
----
$ kubectl --context $CTX_PRIMARY -n kuadrant-coredns rollout status deployment/kuadrant-coredns
----
+
.Example output
[source,text]
...
[source,bash]
----
$ kubectl --context $CTX_PRIMARY -n kuadrant-coredns create secret generic coredns-credentials \
...
[source,terminal]
----
$ kubectl --context $CTX_PRIMARY -n kuadrant-coredns logs deployment/kuadrant-operator
----
+
Expect to see `Registered DNS provider: `kuadrant-operator-controller-manager`.
