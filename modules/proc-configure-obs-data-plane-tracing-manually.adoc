...
* You have the Tempo trace collector installed and configured to support OpenTelemetry.
//Q:does this mean we completed steps 1 & 2 of https://docs.redhat.com/en/documentation/red_hat_openshift_service_mesh/3.2/html/observability/distributed-tracing-and-service-mesh#ossm-config-otel_ossm-traces

.Procedure

//do we still need to configure telemetry in service mesh?
...
. Configure a tracing extension provider for {service-mesh} in your `Istio` CR by adding a list value to the `spec.values.meshConfig.extensionProviders` parameter:
//Q: did we also add the otel port and service info in this step?
+
...
====
//this isn't what I see here: https://docs.redhat.com/en/documentation/openshift_container_platform/4.21/html-single/red_hat_build_of_opentelemetry/index#otel-collector-config-options_otel-configuration-of-otel-intro
//info correct?
